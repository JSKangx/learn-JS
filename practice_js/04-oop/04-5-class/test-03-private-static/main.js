...
// private 개념으로 사용하고자 하는 멤버의 이름을 #으로 시작하면 외부에서 접근하지 못한다.
class User {
  #name;
  age;
  constructor(name, age) {
    // name이 아닌 #name으로 써야 한다.
    // #name이 클래스 필드에 선언되어 있지 않다면, 최초로 생성자 내부에서는 선언이 불가하다.
    this.#name = name;
    this.age = age;
  }
  #myFun() {
...
// user.#name = "Betty"; // 외부에서 접근 불가. Uncaught SyntaxError: Private field '#name' must be declared in an enclosing class
user.age = 30; // 외부에서 접근/변경 가능
user.sayHello(); // (1) Hello, Alberto(30), (2) this.#myFun() 둘다 실행 가능
...
// this.data1은 undefined다. data1은 클래스 메모리에 없다. 객체를 생성했다고 하더라도 data1은 생성된 객체의 메모리에만 업로드 된다.
...
// 객체 멤버는 반드시 객체 생성 후 객체명으로 접근해야 한다.
let obj = new MyClass();
